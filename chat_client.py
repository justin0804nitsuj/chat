...
# 伺服器設定（測試用）
SERVER_HOST = "127.0.0.1"  # 根據需求修改
SERVER_PORT = 12345

DATA_FILENAME = "messages.json"
PROFILE_FILENAME = "profile.json"

# 檔案大小門檻 (byte)，超過此值則採用分塊上傳
CHUNK_THRESHOLD = 1048576  # 1MB

class ChatClientApp:
    def __init__(self, root):
        self.root = root
        self.root.title("聊天室 - 分塊上傳超大檔案 + 就地編輯等")
        ...
        self.day_frames = {}         # 每天的容器
        self.last_header_info = {}   # {date: (last_sender, count)}
        self.ephemeral_map = {}      # 存放每則訊息的 UI 元件
        ...
        self.search_entry = tk.Entry(search_frame, textvariable=self.search_var,
                                     font=("Arial", 16), bg="#3a3a3a", fg="white",
        ...
        # -------------------- 預覽區 (附件) --------------------
        self.preview_label = tk.Label(self.root, text="", fg="white", bg="#1f1f1f",
                                      font=("Arial", 20))
    ...
    def play_video(self, file_path):
        ...
        video_win.title("影片播放")
        video_win.geometry("640x480")
        
        # 建立一個 Label 來顯示影片
        video_label = tk.Label(video_win)
        video_label.pack(expand=True, fill="both")
        
        # 建立 tkvideo 播放器，設定影片路徑、顯示 Label 與播放大小，並設置循環播放
        player = tkvideo(file_path, video_label, loop=1, size=(640,480))
        player.play()
    ...
    def send_file_in_chunks(self, file_path):
        ...
        # 畫出一個完整的白色圓圈作為背景（未上傳部分保持白色）
        canvas.create_oval(x0, y0, x1, y1, outline="white", width=2)
        # 在圓圈上建立藍色 arc，起始角度設為 270（底部），extent 初始為 0
        arc = canvas.create_arc(x0, y0, x1, y1, start=270, extent=0, style="arc", outline="blue", width=2)

        time_label = tk.Label(progress_win, text="剩餘時間：--秒", font=("Arial", 12))
    ...
    def get_day_frame(self, date_str):
        if date_str in self.day_frames:
            ...
            date_label = tk.Label(day_frame, text=f"=== {date_str} ===", bg="#2b2b2b", fg="white", font=("Arial",25,"italic"))
